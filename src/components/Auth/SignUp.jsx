...
const SignUp = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!firstName || !lastName || !email || !phone || !password || !confirmPassword) {
            toastRef.current?.showError("All fields are required!");
            return;
        }

        if (!isValidEmail(email)) {
            toastRef.current?.showError("Invalid email format!");
            return;
        }

        if (!isValidPhone(phone)) {
            toastRef.current?.showError("Invalid phone number! Must be 10 digits.");
            return;
        }
        ...
        try {
            ...
            if (result.success) {
                toastRef.current?.showSuccess("Registration successful! Redirecting...");
                setTimeout(() => navigate("/login"), 2000);
            } else {
                toastRef.current?.showError(result.message || "Registration failed!");
            }
        } catch (error) {
